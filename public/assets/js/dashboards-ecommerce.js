...
(function () {
  ...
  if (typeof profitLastMonthEl !== undefined && profitLastMonthEl !== null) {
    profitLastMonth = new ApexCharts(profitLastMonthEl, profitLastMonthConfig);
    profitLastMonth.render();
  }

  const updateProfitLastMonthChart = (newData) => {
    if () {
      profitLastMonth.updateSeries([{ data: newData }]);
    }
  };

  // Example AJAX call
  const fetchDataProfitLastMonth = () => {
    $.get('/dashboard/dashboard-profit-last-month', {})
      .done((data) => {
        updateProfitLastMonthChart(data);
      })
