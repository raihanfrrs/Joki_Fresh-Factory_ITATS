...
(function () {
  ...
  const weeklyEarningReportsEl = document.querySelector('#weeklyEarningReports'),
    weeklyEarningReportsConfig = {
      ...
      xaxis: {
        categories: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'June', 'July', 'Aug', 'Sept', 'Nov', 'Dec'],
        axisBorder: {
          show: false
        },
        axisTicks: {
  ...
  const updateWeeklyEarningReportChart = (newData) => {
    if (weeklyEarningReports) {
      weeklyEarningReports.updateSeries([{ data: newData }]);
    }
  };

  // Example AJAX call
  const fetchDataMonthlyEarningReport = () => {
    $.get('/dashboard/monthly-earning-report', {})
      .done((data) => {
        updateWeeklyEarningReportChart(data);
      })
